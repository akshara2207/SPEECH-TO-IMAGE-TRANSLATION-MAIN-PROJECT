(function () {
  ...
  micDiv = document.getElementById("micDiv");
  confirmDiv = document.getElementById("confirmDiv");
  generateBtn = document.getElementById("generateBtn");

  let oldContent = generateBtn.innerHTML;
  generateBtn.addEventListener("click", () => {
    generateBtn.innerHTML = "Listening...";
    generateBtn.disabled = true;

    fetch("/get_speech_text/")
      .then((response) => response.json())
      .then((data) => {
        generateBtn.innerHTML = oldContent;
        generateBtn.disabled = false;
        if (data.text === "") {
          return;
        }
        document.getElementById("speechText").textContent = data.text;
        micDiv.classList.toggle("d-none");
        confirmDiv.classList.toggle("d-none");
      });
  });

  document.getElementById("confirmForm").addEventListener("reset", (e) => {
    micDiv.classList.toggle("d-none");
    confirmDiv.classList.toggle("d-none");
  });
})();
